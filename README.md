...
| `int` | `Read` | `uint8_t* payload` | Initiates an I2C read sequence. Response is copied to `payload` array. No parsing of the received message is done and no `Message` is created. <BR> **CAUTION:** The user must ensure that sufficient space is available in `payload` to hold the complete I2C message. <BR> *Recommendation:* For reading raw ASN.1 messages it is advised to use the `ReceiveRawMessage` method instead. | Error code according to the Atmel data sheet if an Atmel platform is used. 0 for success. |
| `int` | `Write` | `uint8_t* payload` | Initiates an I2C write sequence. Data from `payload` array is sent. <BR> *IMPORTANT:* For a successful write to the sensor, it is expected that `payload[0]` = `0xEE` and `payload[1]` = length of the subsequent ASN.1 message to send. <BR> *Recommendation:* For sending raw ASN.1 messages, it is advised to use `SendRawMessage()` instead. | Error code according to the Atmel data sheet if an Atmel platform is used. 0 for success. |
...
| `bool` | `GetPlatformInformation` | None | Reads firmware version and MCU ID from sensor and stores the data in class attributes `FirmwareVersionMajor`, `FirmwareVersionMinor`, `MCUUniqueIdentifier`. This method is automatically called as part of `Start()` method. | `true` if read succeeded, otherwise `false`. | 
...
| --- | --- | --- |
| `uint8_t` | `FirmwareVersionMajor` | Sensor firmware version, major. Read when sensor is initialized by calling `Start()` method or when calling method `GetPlatformInformation()`. |
| `uint8_t` | `FirmwareVersionMinor` | Sensor firmware version, minor. Read when sensor is initialized by calling `Start()` method or when calling method `GetPlatformInformation()`. |
| `char*` | `MCUUniqueIdentifier` |  Pointer to string holding the sensor's MCU ID. Id is read from sensor is initialized by calling `Start()` method or when calling method `GetPlatformInformation()`. |