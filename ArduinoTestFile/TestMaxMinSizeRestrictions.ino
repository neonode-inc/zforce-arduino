...
//------------------------------------
  // Send and read SizeRestriction
  zforce.SizeRestriction(true,100,true,100);
  ...
  if (msg->type == MessageType::SIZERESTRICTIONTYPE)
  {
    Serial.print("Message type is: ");
    Serial.println((int)msg->type);
    
    Serial.print("sizeMaxEnabled is: ");
    Serial.println(((SizeRestrictionMessage*)msg)->maxSizeEnabled);
    Serial.print("sizeMax is: ");
    Serial.println(((SizeRestrictionMessage*)msg)->maxSize);
    Serial.print("sizeMinEnabled is: ");
    Serial.println(((SizeRestrictionMessage*)msg)->minSizeEnabled);
    Serial.print("sizeMin is: ");
    Serial.println(((SizeRestrictionMessage*)msg)->minSize);
    Serial.println("Received sizerestriction Response");
  ...
  if (touch != NULL)
  {
    if (touch->type == MessageType::TOUCHTYPE)
    {
      for (uint8_t i = 0; i < ((TouchMessage*)touch)->touchCount; i++)
      {
        ...
        Serial.print("ObjectSize is: ");
        Serial.println(((TouchMessage*)touch)->touchData[i].sizeX);
      }
    }
